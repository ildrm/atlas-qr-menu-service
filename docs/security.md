...
- Mutating cookie-authenticated requests require matching CSRF/origin state.
...
- PostgreSQL hardening adds tenant indexes and RLS policies for defense in depth.
...
| CSRF                     | SameSite cookie, CSRF header/cookie and origin checks                  | Test all proxy/origin combinations in staging                                       |
| Cross-tenant IDOR        | membership guard, permission guard, ownership predicates, RLS policies | Run dedicated tenant-isolation integration/fuzz suite with restricted DB role       |
...
`.env.example` contains development placeholders only. Production must inject secrets from a managed secret store. Rotate session pepper and CSRF secrets through a dual-key transition, not an immediate cutover that invalidates all sessions without intent. Never log cookies, tokens, password bodies, API keys, provider secrets, or full client IP addresses.
...
Do not expose the deployment publicly until TLS, trusted-proxy configuration, restricted database roles/RLS context, secret management, backups/restore test, monitoring/alerting, email-domain controls, rate-limit storage, dependency scanning, and a tenant-isolation test suite are active.
